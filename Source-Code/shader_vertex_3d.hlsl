/*==============================================================================
   ...
   3D描画用頂点シェーダー [shader_vertex_3d.hlsl] (Phong Model)
...
VS_OUT main(VS_IN vi)
{
    ...
    if (useSkinning != 0 && weightSum > 0.0001f)
    {
        // スキニングあり
        skinnedPos = 0.0f;
        skinnedNormal = 0.0f;
    ...
    if (useSkinning != 0 && weightSum > 0.0001f)
    {
        uint idx = vi.boneIndex.x;
        float w = vi.boneWeight.x;
        
        if (idx >= MAX_BONES || w < 0.0001f)
        {
            skinnedPos = localPos;
            skinnedNormal = localNormal;
            skinnedTangent = localTangent;
        ...
        else
        {
            ...
            skinnedPos = mul(localPos, M);
            
            float3 n = mul(float4(localNormal, 0.0f), M).xyz;
            skinnedNormal = normalize(n);
            
            float3 t = mul(float4(localTangent, 0.0f), M).xyz;
            skinnedTangent = normalize(t);
        }
    }
    else
    {
